...
This program has been written on Mac OS El Capitan, but a fresh installation test could be done on Ubuntu only. Mac users can safely follow the instructions below (skipping the [Prerequisites](###_Prerequisites) section), but in case of problems we did not keep track (yet) of the solutions here
...
(see also the section [Installation Notes](##_Installation_Notes))
