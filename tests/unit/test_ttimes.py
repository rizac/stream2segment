...
# 
    ...
    def test_ttables(self):
        # see end of the loop to understand why we set these vars:
        cubic_median_is_better = 0
        cubic_median_is_worse = 0
        cubic_mean_is_better = 0
        cubic_mean_is_worse = 0
        for ttable in self.ttables:
            ...
            real_results = []
            for v in values:
                real_results.append(get_min_travel_time(v[0], v[2], ttable._phases.tolist(),
                                    v[1], str(ttable._modelname)))
