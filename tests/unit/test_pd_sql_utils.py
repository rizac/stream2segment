...
class Test(unittest.TestCase):
    ...
    def test_add2sb_double_single_matching_col(self):
        ...
        newd, discarded, new = add2db(d, self.session, [Customer.id])
        assert discarded == 0
        assert new == 0
        ...
        dlen = len(d)
        newd, discarded, new = add2db(d, self.session, [Customer.id], drop_duplicates=False)
    ...
    def test_add2sb_double_matching_cols(self):
        ...
        dlen = len(d)
        newd, discarded, new = add2db(d, self.session, [Customer.id, Customer.time])
        assert discarded == 0
        assert new == 0
        assert dlen == len(newd)
        ...
        d = pd.DataFrame([{'id': 1, 'time': d2009}, {'id': 22, 'time': d2009}, {'id': 3, 'time': d2008}])
        dlen = len(d)
        newd, discarded, new = add2db(d, self.session, [Customer.id, Customer.time], add_buf_size=1)
        assert discarded == 1
        assert new == 1
        assert len(newd) == dlen - 1
        ...
        dlen = len(d)
        newd, discarded, new = add2db(d, self.session, [Customer.id, Customer.time], drop_duplicates=False)
        assert discarded == 0
        assert new == 1
        assert len(newd) == dlen
        ...
        dlen = len(d)
        newd, discarded, new = add2db(d, self.session, [Customer.id, Customer.time], drop_duplicates=True)
        assert discarded == 1  # the second {'id': 22, 'time': d2009}
...
#         4    a 2006-01-01  1.0
    ...
    def test_dfupdate1(self):
        ...
        d2 = dfupdate(d, dnew, ['id', 'name'], ['time'])
        ...
        # for times, as numpy is weird about that and dfupdate sets the values of a column (np array)
        # check also that types are the same
        assert d2['time'].dtype == dnew['time'].dtype
        g = 9
        
    def test_dfupdate2(self):
        d2006 = datetime(2006, 1, 1)
        d2007 = datetime(2007, 12, 31)
        d2008 = datetime(2008, 3, 14)
        d2009 = datetime(2009, 9, 25)
        ...
        # dnew has a dupe (last two elements) we will remove them in dfupdate
        dnew = pd.DataFrame([{'id':45, 'name': 'a', 'time': None, 'a':5}, {'id':45, 'name': 'b', 'time': d2006, 'a':5},
                          {'id':45, 'name': 'c', 'time': d2009, 'a':5}, {'id':45, 'name': 'd', 'time': None, 'a':5},
