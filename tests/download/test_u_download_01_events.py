...
"""  # noqa
        ...
        # mock ThreadPool (tp) to run one instance at a time, so we get deterministic
        # results:
    ...
    def setup_urlopen(self, urlread_side_effect):
        ...
        for k in urlread_side_effect:
            mymock = Mock()
            if type(k) == int:
                mymock.read.side_effect = HTTPError('url', int(k), responses[k],
                                                    None, None)
    ...
    def get_events_df(self, url_read_side_effect, session, url, evt_query_args,
                      start, end, db_bufsize=30, timeout=15, show_progress=True):
...
"""  # noqa
        ...
        # two rows discarded: LEGACY CODE, not true anymore: the parsing happens after
        # fetching data, which raised, so no parsing and thus no discarded rows:
        # assert "2 row(s) discarded" in log2

        # We called fdsn_url once, plus two times because we split url time bounds once,
        ...
        with pytest.raises(FailedDownload) as fld:
            data = self.get_events_df(urlread_sideeffect, db.session, "http://eventws",
                                      {}, datetime.utcnow() - timedelta(seconds=1),
                                      datetime.utcnow(), db_bufsize=self.db_buf_size)
        # assert we got the same result as above:
        assert len(db.session.query(Event).all()) == len(pd.unique(data['id'])) == \
               len(data) == 3
        log2 = self.log_msg()
    ...
    def test_get_events_eventws_not_saved(self, db):
        ...
        assert len(db.session.query(WebService.url).filter(
            WebService.url == 'abcd').all()) == 1
        # assert only three events were successfully saved to db (two have same id)
        assert not db.session.query(Event).all()
        # we cannot assert anything has been written to logger cause the exception are
        # caught if we run from main. This should be checked in functional tests where
    ...
    def test_pbar1(self, mock_urljoin, mock_pbar, db):
        ...
        urlread_sideeffect = [
            '''20160508_0000129|2016-05-08 05:17:11.500000|40.57|52.23|60.0|AZER|EMSC-RTS|AZER|505483|ml|3.1|AZER|CASPIAN SEA, OFFSHR TURKMENISTAN'''  # noqa
        ]
        mock_pbar.reset_mock()
        mock_pbar.return_value.updates = []
        _ = self.get_events_df(urlread_sideeffect, db.session, "abcd", {},
        ...
        assert mock_pbar.call_args[0][0] < self.get_pbar_total_steps()
        assert sum(mock_pbar.return_value.updates) == mock_pbar.call_args[0][0]
        # assert that we do not have minmagnitude in the first two sub-request
        # (from index 1), but in the third (do not test other sub requests)
        req_kwargs = [_[1] for _ in mock_urljoin.call_args_list]
...
"""  # noqa
        ...
        # 1 row(s) discarded (malformed server response data, e.g. NaN's).
        # url: file:////private/var/folders/l9/zpp7wn1n4r7bt4vs39gylk4w0000gn/...txt
...
"""  # noqa
        ...
        # (Note that `urlread_sideeffect` above should raise a socket timeout and an
        # THHP error 500, but the first socket.timeout forces by design the split of the
        # request into sub-request, so the first exception caught is the HTTP error 500
        with pytest.raises(FailedDownload) as fdl:
        ...
        for url_read_side_effect in [b'', b'!invalid!']:
            expected_exc = NothingToDownload if not url_read_side_effect \
                else FailedDownload
            expected_err = ERR_FETCH_NODATA if not url_read_side_effect \
                else ERR_FETCH_FDSN
