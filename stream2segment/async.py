...
def url_read(url, blocksize=-1, decode=None, on_exc=None, **kwargs):
    """
        ...
        decode is specified). Returns None if on_exc is specified and an IOException is thrown,
        otherwise raises it
...
def read_async(urls, onsuccess, onerror, max_workers=5, blocksize=1024*1024, decode=None,
    ...
    """
        ...
        below)
        :param urls: an iterable of strings denoting the urls to read
        ...
        `data` is the data read from `url` and `index` is an incrementing integer
        (``index < len(urls)``) useful for, e.g., displaying progress. Note that due to threading
        it is **not** guaranteed that the index-th element of `urls` is `url`.
        ...
        ```urllib2.HTTPError, urllib2.URLError, httplib.HTTPException```
