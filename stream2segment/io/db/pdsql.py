...
#             suffix += '_'
#         for db_df in db2dfs(engine, stmt, chunksize=chunksize):
#             if db_df.empty:
#                 continue
...
def insert_df(
    ...
    while start < len(dfr):
        ...
        index_inserted.update(  # Python Set update, not SQL!
            r[id_col] for r in execute_sql(engine, stmt, rows)  # <- here SQL
        )
        start += chunksize
...
def select_df(engine, query: Select, chunksize=20000) -> Iterable[pd.DataFrame]:
