...
def save_stationxml(
    *,
    ...
    def url_iterator(dfr: pd.DataFrame) -> Iterable[str]:
        ...
        for net, sta, ws_id, ws_url, sta_id in zip(
            dfr[net_col],
            dfr[sta_col],
            dfr[ws_id_col],
            dfr[url_col],
...
def save_quakeml(
    *,
    ...
    def url_iterator(dfr: pd.DataFrame) -> Iterable[str]:
        ...
        for db_ev_id, cat_ev_id, ws_url in zip(
            dfr[Event.id.key],
        ...
        for dfr in fetch_df(engine, stmt):
            ...
            for response in reader:
                ...
                if not response.is_ok or response.status_code == 204:
                    ...
                    msg = responses.get(response.status_code, "Unknown error")
                    logger.warning(
...
#                             conn_nested.execute(insert_stmt, {
#                                 'id': db_ev_id,
#                                 'data': response.data
#                             })
#                         saved += 1
