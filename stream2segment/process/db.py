"""
Object Relational Models (ORM) for processing.
...
from stream2segment.io.db.models import (Segment, Station, Base,
...
def get_session(dburl, scoped=False, **engine_args):
    ...
    **process downloaded data**
    ...
    :param dbpath: the path to the database, e.g.
        sqlite:///path_to_my_dbase.sqlite
    ...
    # methods.
    # Solution 1: we might do it already in the io.db.models module, but there
    # we do not want to keep things clean and avoid importing anything so that
    # the module can be portable (in case) to new projects.
    # Solution 2: we might then write here a new class Segment2 extending
    # 'models.Segment' but SQLAlchemy is not designed to handle conditional
    # ORMs like this, and also this implementation might introduce bugs due to
    # the 'wrong' Segment class used. We need thus to programmatically toggle
    # on/off the new methods: what we found here is maybe not the cleanest
    # solution, but it's currently the only one with no overhead: simply attach
    # methods to the Segment class. Call manually
    # '_toggle_enhance_segment(False) to detach the methods when and if you
    # need to (it should never be the case)
...
def _toggle_enhance_segment(value):
    ...
    if value:
        ...
        Segment.inventory = \
            lambda self, *a, **kw: self.station.inventory(*a, **kw)
