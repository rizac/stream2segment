...
def analyze_coda(stream, fm=6, cycle=10, niveau_bruit=16, Lw=50, noise_duration=5, subwdw_length=5,
                 subwdw_length_rec=2.5):
    """ 
        Returns the indices of each subwindow corresponding to the sliding mean on that window
    ...
    for st in stream:
        ...
        if (st.stats.npts*st.stats.delta) > 100:
            ...
            if len(stalta) > 0:  # si on detecte effectivement du signal dans la fenetre de bruit: ca va pas
                ret.append(None)  # on ne peut pas definir une bonne moyenne de bruit
            else:
                databruit = bruit
