...
def main(segment, config):
    ...
    *  `obspy.core.Trace <https://docs.obspy.org/packages/autogen/obspy.core.trace.Trace.html#obspy.core.trace.Trace>_`
    ...
    required_amplitudes = np.interp(np.log10(required_freqs), np.log10(ampspec_freqs), normal_spe) / segment.sample_rate
...
def _synth_wa(segment, config, trace_input_type=None):
    ...
    The dict ``config['simulate_wa']`` must be
    implemented and houses the wood-anderson configuration 'sensitivity', 'zeros', 'poles' and 'gain'
