...
EMPTY_RESPONSE = 'Response empty (0 bytes)'


def DF_ITEM_DISCARDED_(num):
    return ("{:d} {} discarded (malformed or invalid, "
            "e.g. with NaN/None values)").format(num, "item" if num == 1 else "items")

DB_NOITEM_SAVED = 'No item saved'


def DB_NEWITEM_SAVED_(num):
    return '{:d} new {} saved'.format(num, "item" if num == 1 else "items")
...
def format(exc_or_msg, url=None):  # @ReservedAssignment # pylint:disable=redefined-builtin
    ...
    if isinstance(exc_or_msg, Exception):
        exc_or_msg = "{}: {}".format(exc_or_msg.__class__.__name__, str(exc_or_msg))
    if url:
        exc_or_msg = "{}. URL: {}".format(exc_or_msg, url)
    return exc_or_msg
